...
function hitungBahan() {
  ...
  function tampilkanVisual() {
    ...
    var baseWidth = 500;
    var baseHeight = baseWidth * (panjangBahanTotal / lebarBahan);
    ...
    for (var i = 0; i < jumlahBarisPola; i++) {
        for (var j = 0; j < jumlahPotonganPerlebar; j++) {
            if (nomorPola <= jumlahPotonganTotal) {
                ...
                svg += `<rect x="${x}" y="${y}" width="${lebarPola * scaleX}" height="${panjangPola * scaleY}" fill="#3b82f6" stroke="#2563eb" stroke-width="1"/>`;
                svg += `<text x="${x + (lebarPola * scaleX / 2)}" y="${y + (panjangPola * scaleY / 2)}" fill="white" text-anchor="middle" dominant-baseline="middle" font-size="10">${nomorPola}</text>`;
                nomorPola++;
            }
        }
    }
    ...
    if (sisaBahanPerlebar > 0) {
        var x = jumlahPotonganPerlebar * lebarPola * scaleX;
        var y = 0;
        svg += `<rect x="${x}" y="${y}" width="${sisaBahanPerlebar * scaleX}" height="${jumlahBarisPola * panjangPola * scaleY}" fill="#fca5a5" stroke="#ef4444" stroke-width="1"/>`;

        var tambahanPola = Math.floor((jumlahBarisPola * panjangPola) / panjangPola);
        var nomorTambahanPola = 1;
        for (var k = 0; k < tambahanPola; k++) {
            y = k * panjangPola * scaleY;
            if ((k + 1) * lebarPola <= sisaBahanPerlebar) {
                svg += `<rect x="${x}" y="${y}" width="${lebarPola * scaleX}" height="${panjangPola * scaleY}" fill="#fcd34d" stroke="#f59e0b" stroke-width="1"/>`;
                svg += `<text x="${x + (lebarPola * scaleX / 2)}" y="${y + (panjangPola * scaleY / 2)}" fill="black" text-anchor="middle" dominant-baseline="middle" font-size="10">${nomorPola + nomorTambahanPola}</text>`;
                nomorTambahanPola++;
            }
        }
    }

    svg += `<text x="${baseWidth / 2}" y="${baseHeight + 20}" text-anchor="middle" font-size="12">Lebar Bahan: ${lebarBahan} cm</text>`;
    svg += `<text x="-20" y="${baseHeight / 2}" text-anchor="middle" font-size="12" transform="rotate(-90 -20 ${baseHeight / 2})">Panjang Bahan: ${(jumlahBarisPola * panjangPola).toFixed(2)} cm</text>`;

    svg += `<rect x="10" y="${baseHeight + 30}" width="15" height="15" fill="#3b82f6"/>`;
    svg += `<text x="30" y="${baseHeight + 42}" font-size="12">Pola (${lebarPola}x${panjangPola} cm)</text>`;
    svg += `<rect x="150" y="${baseHeight + 30}" width="15" height="15" fill="#fca5a5"/>`;
    svg += `<text x="170" y="${baseHeight + 42}" font-size="12">Sisa Bahan (${sisaBahanPerlebar.toFixed(2)} cm)</text>`;
    svg += `<rect x="290" y="${baseHeight + 30}" width="15" height="15" fill="#fcd34d"/>`;
    svg += `<text x="310" y="${baseHeight + 42}" font-size="12">Tambahan Pola dari Sisa Bahan</text>`;
    ...
    container.style.height = `${container.offsetWidth * (panjangBahanTotal / lebarBahan)}px`;

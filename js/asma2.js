...
function generateQuoteByNumber() {
    ...
    if (asmaulHusna.length === 0) {
        console.log("asmaulHusna is empty");
        alert('Data belum dimuat, coba lagi nanti.');
        return;
    }
    ...
    const randomBackgroundIndex = Math.floor(Math.random() * backgroundCount) + 1;
    console.log("Random background index:", randomBackgroundIndex);
    const backgroundUrl = `url('../../asset/background/background${randomBackgroundIndex}.jpg')`;
    console.log("Background URL:", backgroundUrl);
    document.getElementById('quote-box').style.backgroundImage = backgroundUrl;
...
function hideDetail() {
    ...
    console.log('quote-detail element:', quoteDetail);
    if (quoteDetail) {
      quoteDetail.style.display = 'none';
    } else {
      console.log('quote-detail element not found');
    }
  }
